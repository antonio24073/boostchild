 ...
 */
function theme_boostchild_get_loginbackgroundimage_files() {
    ...
    if ($files == null) {
        ...
        $files = $fs->get_area_files($systemcontext->id, 'theme_boostchild', 'loginbackgroundimage',
            false, 'itemid', false);
    }
 ...
 * @throws dml_exception
 */
function theme_boostchild_get_random_loginbackgroundimage_number() {
    ...
    if ($number == null) {
        ...
        $files = theme_boostchild_get_loginbackgroundimage_files();
 ...
 * @return string
 */
function theme_boostchild_get_random_loginbackgroundimage_class() {
    // Get the static random number.
    $number = theme_boostchild_get_random_loginbackgroundimage_number();
 ...
 * @throws dml_exception
 */
function theme_boostchild_get_loginbackgroundimage_text() {
    ...
    if ($number != null) {
        ...
        $lines = explode("\n", get_config('theme_boostchild', 'loginbackgroundimagetext'));
 ...
 * @return string
 */
function theme_boostchild_get_loginbackgroundimage_scss() {
    $count = 0;
    $scss = "";
 ...
 * @return array
 */
function theme_boostchild_get_imageareacontent() {
    // Get cache.
    $themeboostcampuscache = cache::make('theme_boostchild', 'imagearea');
    ...
    if (!empty($cachecontent)) {
        ...
        $fs = get_file_storage();
        // Get all files from filearea.
        $files = $fs->get_area_files($systemcontext->id, 'theme_boostchild', 'imageareaitems', false, 'itemid', false);
        ...
        if (!empty($files)) {
            ...
            $lines = explode("\n", get_config('theme_boostchild', 'imageareaitemsattributes'));
            // Parse item settings.
            foreach ($lines as $line) {
                $line = trim($line);
 ...
 */
function theme_boostchild_process_flatnav(flat_navigation $flatnav) {
    global $USER;
    // If the setting defaulthomepageontop is enabled.
    if (get_config('theme_boostchild', 'defaulthomepageontop') == 'yes') {
        ...
        if (($coursehomenode = $flatnav->find('coursehome', global_navigation::TYPE_CUSTOM)) != false) {
            ...
            if (get_config('core', 'defaulthomepage') == HOMEPAGE_SITE) {
                ...
                $flatnavreturn = theme_boostchild_set_node_on_top($flatnav, 'home', $coursehomenode);
            } else if (get_config('core', 'defaulthomepage') == HOMEPAGE_MY) { // If the dashboard is set as the default homepage
                // by the admin.
                // Return the modified flat_navigation.
                $flatnavreturn = theme_boostchild_set_node_on_top($flatnav, 'myhome', $coursehomenode);
 ...
 * @return flat_navigation.
 */
function theme_boostchild_set_node_on_top(flat_navigation $flatnav, $nodename, $beforenode) {
    ...
    if (get_config('theme_boostchild', 'showsettingsincourse') == 'yes') {
        ...
        if ($PAGE->context->contextlevel == CONTEXT_COURSE || $PAGE->context->contextlevel == CONTEXT_MODULE) {
            ...
            if (!empty($node)) {
                ...
                if (((get_config('theme_boostchild', 'incoursesettingsswitchtoroleposition') == 'yes') ||
                    (get_config('theme_boostchild', 'incoursesettingsswitchtoroleposition') == 'both'))
                    ...
                    if (is_array($roles) && (count($roles) > 0)) {
                        ...
                        $properties = array('text' => get_string('switchroleto', 'theme_boostchild'),
 ...
 * @return navigation_node.
 */
function theme_boostchild_get_incourse_activity_settings() {
    global $PAGE;
 ...
 * @return string.
 */
function theme_boostchild_get_course_guest_access_hint($courseid) {
    global $CFG;
    ...
    foreach ($instances as $instance) {
        ...
        if ($plugin->show_enrolme_link($instance)) {
            $html = html_writer::tag('div', get_string('showhintcourseguestaccesslink',
                'theme_boostchild', array('url' => $CFG->wwwroot . '/enrol/index.php?id=' . $courseid)));
            break;
        }
    }
 ...
 * @return boolean
 */
function theme_boostchild_show_banner_on_selected_page($infobannerpagestoshow, $infobannercontent, $thispagelayout,
 ...
 * @return boolean
 */
function theme_boostchild_show_timed_banner_on_selected_page($now, $timedibshowonpages, $timedibcontent, $timedibstartsetting,
    ...
    if (($now >= $timedibstart && $now <= $timedibend ||
        ...
        $timedinfobannershowonselectedpage = theme_boostchild_show_banner_on_selected_page($timedibshowonpages,
 ...
 * @return string.
 */
function theme_boostchild_get_course_information_banners() {
    ...
    if (get_config('theme_boostchild', 'showhintcoursehidden') == 'yes'
            && has_capability('theme/boostchild:viewhintinhiddencourse', \context_course::instance($COURSE->id))
            && $PAGE->has_set_url()
        ...
        $html .= get_string('showhintcoursehiddengeneral', 'theme_boostchild', $COURSE->id);
        // If the user has the capability to change the course settings, an additional link to the course settings is shown.
        if (has_capability('moodle/course:update', context_course::instance($COURSE->id))) {
            $html .= html_writer::tag('div', get_string('showhintcoursehiddensettingslink',
                    'theme_boostchild', array('url' => $CFG->wwwroot.'/course/edit.php?id='. $COURSE->id)));
        }
        $html .= html_writer::end_tag('div');
        $html .= html_writer::end_tag('div');
        $html .= html_writer::end_tag('div');
    ...
    // intended.
    if (get_config('theme_boostchild', 'showhintcourseguestaccess') == 'yes'
        ...
        $html .= html_writer::end_tag('div');
        $html .= html_writer::start_tag('div', array('class' => 'media-body align-self-center'));
        $html .= get_string('showhintcourseguestaccessgeneral', 'theme_boostchild',
                array('role' => role_get_name(get_guest_role())));
        $html .= theme_boostchild_get_course_guest_access_hint($COURSE->id);
        $html .= html_writer::end_tag('div');
        $html .= html_writer::end_tag('div');
        $html .= html_writer::end_tag('div');
    }
    ...
    // capability "theme/boostchild:viewhintcourseselfenrol".
    if (get_config('theme_boostchild', 'showhintcourseselfenrol') == 'yes'
            && has_capability('theme/boostchild:viewhintcourseselfenrol', \context_course::instance($COURSE->id))
            && $PAGE->has_set_url()
            && $PAGE->url->compare(new moodle_url('/course/view.php'), URL_MATCH_BASE)
            && $COURSE->visible == true) {
        ...
        if (!empty($selfenrolinstances) &&
            ...
            if ($selfenrolmentpossiblecurrently == true) {
                $html .= get_string('showhintcourseselfenrolstartcurrently', 'theme_boostchild');
            } else if ($selfenrolmentpossiblefuture == true) {
                $html .= get_string('showhintcourseselfenrolstartfuture', 'theme_boostchild');
            }
            ...
            if (has_capability('enrol/self:config', \context_course::instance($COURSE->id))) {
                $html .= html_writer::empty_tag('br');
                $html .= get_string('showhintcourseselfenrolinstancecallforaction', 'theme_boostchild');
            }
    ...
    if (get_config('theme_boostchild', 'showswitchedroleincourse') === 'yes') {
        ...
        if (is_role_switched($COURSE->id)) {
            ...
            $html .= get_string('switchedroleto', 'theme_boostchild');
